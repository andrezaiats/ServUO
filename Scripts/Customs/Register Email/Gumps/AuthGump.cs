                ...
                case 1:
                    {
                        ...
                        if (CanValidate(s, key, from))
                        {
                            ...
                            string msg = "Congragulations on sucessfully registering your email address. You will now be able to receive special announcements via email and use the in game commands \"[account\" to manage your password.\n \n" + "Welcome aboard!\nUOShard.com Team";

                            EmailEventArgs eea = new EmailEventArgs(true, null, email, "Email Registered", msg);
                            
                            RegisterEmailClient.SendMail(eea);
                            from.SendMessage("Thank you for registering your email. Checkout your backpack for your prize!");

			    from.AddToBackpack(new StatCapScroll(240));
			    PowerScroll PS = PowerScroll.CreateRandomNoCraft(5, 10);
			    from.AddToBackpack(PS);
                            PowerScroll PS2 = PowerScroll.CreateRandomNoCraft(5, 10);
                            from.AddToBackpack(PS2);
                            PowerScroll PS3 = PowerScroll.CreateRandomNoCraft(5, 10);
                            from.AddToBackpack(PS3);
                            PowerScroll PS4 = PowerScroll.CreateRandomNoCraft(5, 10);
                            from.AddToBackpack(PS4);
                            PowerScroll PS5 = PowerScroll.CreateRandomNoCraft(5, 10);
                            from.AddToBackpack(PS5);
                        }
                        else
                        {
